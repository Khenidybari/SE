...
<?php
  ...
  // eto ung number of rows sa elementary table
                                $shs_count = "SELECT id FROM seniorhighschool";
                                $shs_count_run = mysqli_query($conn, $shs_count);
                                $shs_rowcount = mysqli_num_rows($shs_count_run);

                                // eto naman kinukuha nia ung value ng dategraduated - ung distinct is ginagawa nia as 1 ang mga may duplicates
                                $date_graduated_val = "SELECT DISTINCT dategraduated FROM seniorhighschool";
                                ...
                                if ($date_grad_rows > 0) {
                                    ...
                                    while ($date_graduated_row = mysqli_fetch_assoc($date_graduated_val_run)) {
                                        ...
                                        $select_year = "SELECT dategraduated FROM seniorhighschool WHERE dategraduated = $year";
...
<script>
  ...
  const seniorhighchart1 = document.getElementById('myChart1');
  const seniorhighchart2 = document.getElementById('myChart2');
  const seniorhighchart3 = document.getElementById('myChart3');
  let seniorhighchart2instance;
  let seniorhighchart3instance;
  const data1 = {
    labels: date_graduated,
    datasets: [{
      label: 'Senior High School Graduate Per Year',
      data: year_count,
      backgroundColor: [
        'rgba(255, 26, 104, 0.2)',
        'rgba(54, 162, 235, 0.2)',
  ...
  function clickhandler(click){
    ...
    if (points.length) {
      if (seniorhighchart2instance) {
          // If a chart instance already exists, destroy it before creating a new one
          seniorhighchart2instance.destroy();
        }
      ...
      fetch('seniorhighquery.php?value=' + encodeURIComponent(value))
      ...
      .then(data => {
        ...
        data.forEach(item => {
          gendercounter.push(item);
          if (item.strand === 'ABM') {
            data_array.ABM_count++;
          }
          if (item.strand === 'GAS') {
            data_array.GAS_count++;

          }
          if (item.strand === 'HUMSS') {
            data_array.HUMSS_count++;
          }
          if (item.strand === 'STEM') {
            data_array.STEM_count++;
          }
        });
...
// Chart instance ********** 2nd graph ******************
         const data6 = {
    labels: ['ABM','GAS', 'HUMSS', 'STEM'],
    datasets: [{
      label: ['Strand Count Graduated This Year'],
      data:[data_array.ABM_count, data_array.GAS_count,data_array.HUMSS_count,data_array.STEM_count],
      backgroundColor: [
        'rgba(255, 26, 104, 0.2)',
        'rgba(54, 162, 235, 0.2)',
        'rgba(255, 206, 86, 0.2)',
 ...
 seniorhighchart2instance = new Chart(
          seniorhighchart2,
          config6
        );


function chart2clickeventlistener(click) {
  const points = seniorhighchart2instance.getElementsAtEventForMode(click, 'nearest', { intersect: true }, true );

if (points.length) {
    if (seniorhighchart3instance) {
          // If a chart instance already exists, destroy it before creating a new one
          seniorhighchart3instance.destroy();
        }

  const firstPoint = points[0];

  const value = seniorhighchart2instance.data.labels[firstPoint.index];
  ...
  gendercounter.map(item => {
    if (item.strand === value){
      if (item.gender === 'Male'){
        countgender.male_count++
      }
      if (item.gender === 'Female'){
        countgender.female_count++
      }
    }
  });
//                          ************* FOR THIRD CHART **********************
...
const data7 = {
    labels: ['MALE','FEMALE'],
    datasets: [{
      label: ['Graduate This Year'],
      data:[countgender.male_count, countgender.female_count],
      backgroundColor:[
        'rgba(255, 26, 104, 0.2)',
        'rgba(54, 162, 235, 0.2)',
        'rgba(255, 206, 86, 0.2)',
...
seniorhighchart3instance = new Chart(
      seniorhighchart3,
      config7
    );


seniorhighchart2.onclick = clickhandlerforsecondchart;

//                          ************* FUNCTION FOR THIRD CHART **********************

      }

}

seniorhighchart2.onclick = chart2clickeventlistener;
  ...
  seniorhighchart1.onclick = clickhandler;
