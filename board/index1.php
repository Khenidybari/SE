...
<?php
  ...
  $elem_count = "SELECT id FROM elementary";
  $elem_count_run = mysqli_query($conn, $elem_count);
  $elem_rowcount = mysqli_num_rows($elem_count_run);

  // eto naman kinukuha nia ung value ng dategraduated - ung distinct is ginagawa nia as 1 ang mga may duplicates
  $date_graduated_val = "SELECT DISTINCT dategraduated FROM elementary";
  ...
  if($date_grad_rows > 0){
    ...
    while($date_graduated_row = mysqli_fetch_assoc($date_graduated_val_run)){
      ...
      $select_year = "SELECT dategraduated FROM elementary WHERE dategraduated = $year";
...
<script>
  ...
  const data1 = {
    ...
    datasets: [{
      label: 'Elementary Graduate Per Year',
      data: year_count,
      backgroundColor: [
        'rgba(255, 26, 104, 0.2)',
        'rgba(54, 162, 235, 0.2)',
  ...
  function clickhandler(click){
    ...
    if (points.length) {
      ...
      fetch('elementaryquery.php?value=' + encodeURIComponent(value))
